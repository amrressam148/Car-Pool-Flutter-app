...
class _GoHomeScreenState extends State<GoHomeScreen> {
  ...
  List<String> tripIds = [];
  Map<String, GoHomeTrip> tripsData = {}; // Store trip data
  ...
  Future<void> fetchDriverTrips() async {
    try {
      ...
      if (user != null) {
        ...
        DatabaseReference driverTripsRef = databaseReference.child('Go Home Trips/$userId');
  ...
  Future<void> fetchTripDetails(String userId) async {
    try {
      ...
      for (var tripId in tripIds) {
        DatabaseReference tripRef = databaseReference.child('Go Home Trips/$userId/$tripId');
    ...
    return Scaffold(
      ...
      body: tripIds.isEmpty
        ...
        itemBuilder: (context, index) {
          ...
          GoHomeTrip trip = tripsData[tripId] ?? GoHomeTrip(
            tripId: '',
            pickupLocation: '',
            destination: '',
            price: 0.0,
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          primary: tripFinished ? Colors.grey : Colors.blue,
                        ),
                        child: Text(
                          tripFinished ? 'Finished' : 'Finish Trip',
                        ),
                      ),
                    ],
                  ),
